...
class ProductsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $imageName="";
         $imagepath="http://127.0.0.1:454/storage/app/public/uploads/products";
...
public function show_app_product(Request $request)
    ...
    public function search_product(Request $request, $name)
    {
     $pro = Products::where('Product_name', 'LIKE', '%' . $name . '%')->get();
      // $ids = $request->input('ids', []); // via injected instance of Request
      // $items1 = items::whereIn('id', explode(',', $id))->get();
      // $items1 = items::whereIn('id', explode(',', $id->$request->get()));
 ...
 public function show_by_category(Request $request,$cat)
    {
         $pro = Products::where('Product_Category',$cat)->where('Product_status','Approved')->get();
      // $ids = $request->input('ids', []); // via injected instance of Request
      // $items1 = items::whereIn('id', explode(',', $id))->get();
      // $items1 = items::whereIn('id', explode(',', $id->$request->get()));
