...
class PassportController extends Controller
{
    ...
    {
        $input = $request->only(['name', 'email', 'password','c_password','role','status']);
    ...
    public function login(Request $request)
    {
        ...
        if (auth()->attempt($input)) {
            ...
            $user=new User();
            $user=User::where('status','=','Approved')->where('email','=',$input['email'])->get();;
            ...
            return response()->json([
                'success' => false,
                'message' => 'User Status is Pending Waiting for Approval',
                'status' =>  'Pending'
            ], 401);    
